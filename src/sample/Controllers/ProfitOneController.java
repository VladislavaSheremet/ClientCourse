...
public class ProfitOneController {
    ...
    void initialize() {
        ...
        ButtonPROFIT.setOnAction(event -> {
            ...
            System.out.println("Товар, прибыль от продаж которого необходимл рассчитать: ");
            System.out.println(" ID: " + product.getId() + "; Категория: " + product.getCategory() + "; Наименование: " + product.getName());
            ProfitOneData IdData = new  ProfitOneData();
            IdData.setId(product.getId());
            client.write(IdData);
            double Result = client.readDouble();
            TextAreaResult.clear();
            TextAreaResult.appendText("Прибыль от продажи товара \n\"" +product.getName()+ "\" : " + Double.toString(Result)+ "$\n");
        });
